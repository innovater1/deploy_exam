	...
	<body>
		...
		<form name="frmMember" action="./processAddMember.jsp" method="post">
			<div class="form-group row" >
				...
				<div class="col-md-3">
			 	...
			 	<br><input type="button" name="btnIsDuplication2nd" value="ajax 아이디 중복확인" >
				</div>
			</div>
			<div class="form-group row" >
		...
		<script>
			document.addEventListener("DOMContentLoaded", function() {
				const frmMember = document.frmMember; // 폼을 들고 옴.
				...
				btnIsDuplication2nd.addEventListener('click', function() {
					const memberId = frmMember.memberId.value; // 아이디 input에 있는 값.
					xhr.open('GET', 'ajaxIdCheck.jsp?id=' + memberId); // HTTP 요청 초기환. 통신 방식과 url 설정.
					xhr.send(); // url에 요청을 보냄.
					// 이벤트 등록. XMLHttpRequset 객체의 readyState 프로퍼티 값이 변할 때마다 자동으로 호출
